import { getCookie } from "../../utils/utils";
// export const socketMiddleware = (wsActions) => {
//   return (store) => {
//     const { wsInit, wsSendUserOrder, onOpen, onClose, onError, onMessage } = wsActions;
//     let socket = null;

//     return (next) => (action) => {
//       const { dispatch, getState } = store;
//       const { type, payload } = action;
//       const { user } = getState().user;
//       const accessToken = getCookie("token");
//       //console.log(action);
//       if (type === wsInit) {
//         socket = new WebSocket(`wss://norma.nomoreparties.space/orders/all`);
//       }
//       if (type === wsInit && user) {
//         socket = new WebSocket(`wss://norma.nomoreparties.space/orders?token=${accessToken}`);
//       }
//       if (socket) {
//         //вызывается после открытия соединения
//         socket.onopen = (event) => {
//           //console.log("open", event);
//           dispatch({ type: onOpen, payload: event });
//         };

//         socket.onerror = (event) => {
//           //console.log("error", event);
//           dispatch({ type: onError, payload: event });
//         };
//         //вызывается при получении сообщения от сервера. Сообщение находится в свойстве data объекта event.
//         socket.onmessage = (event) => {
//           // console.log("messsage", event);
//           const { data } = event;
//           const parsedData = JSON.parse(data);
//           const { success, ...restParsedData } = parsedData;

//           dispatch({ type: onMessage, payload: restParsedData });
//         };

//         socket.onclose = (event) => {
//           // console.log("close", event);
//           dispatch({ type: onClose, payload: event });
//         };

//         if (type === wsSendUserOrder) {
//           const order = { ...payload, token: accessToken };
//           socket.send(JSON.stringify(order));
//         }
//       }

//       next(action);
//     };
//   };
// };
export const socketMiddleware = (wsActions) => {
  return (store) => {
    let socket = null;
    const { wsInit, wsClose, wsSendUserOrder, onOpen, onClose, onError, onMessage } = wsActions;

    return (next) => (action) => {
      const { dispatch } = store;
      const { type, payload } = action;

      if (type === wsInit) {
        socket = new WebSocket(payload);
        socket.onopen = (event) => {
          dispatch({ type: onOpen, payload: event });
        };

        socket.onerror = (event) => {
          console.log("ошибка", event);
          dispatch({ type: onError, payload: event });
        };

        socket.onmessage = (event) => {
          const { data } = event;
          const parsedData = JSON.parse(data);
          dispatch({
            type: onMessage,
            payload: { data: parsedData, timestamp: new Date().getTime() / 100 },
          });
        };

        socket.onclose = (event) => {
          dispatch({ type: onClose, payload: event });
        };
      }

      if (wsClose && type === wsClose && socket) {
        socket.close();
      }

      if (wsSendUserOrder && type === wsSendUserOrder && socket) {
        socket.send(JSON.stringify(payload));
      }

      next(action);
    };
  };
};
